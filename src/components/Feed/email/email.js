...
const Register = ({ classes, history, match, setCurrentRoute }) => {
  ...
  const handleSubmit = async (event, verifyEmail, client) => {
    ...
    if (!res.data.verifyEmail.isVerified) setOtpError(true);
      ...
      client.writeData({ data: { showEmail: false } });
      history.push("/");
    }
  };
  return (
    <ApolloConsumer>
      {client => {
        if (!cuser.profileSet[0].emailverified) {
          client.writeData({ data: { showEmail: true } });
        }
        return (
          <div className={classes.root}>
            <Paper className={classes.paper}>
              ...
              <Mutation
                ...
                onCompleted={data => {
                  if (data.verifyEmail.isVerified) {
                    client.writeData({ data: { showEmail: false } });
                  }
                }}
              >
                {(verifyEmail, { loading, error, called, client }) => {
                  return (
                    <div>
                      ...
                      <form
                        onSubmit={event =>
                          handleSubmit(event, verifyEmail, client)
                        }
